#!/usr/bin/env python3
# S91 CLOSE — run from ~/abletrace-lab-docs on the Mac.
#   python3 patch_s91_close.py
# Asserts every anchor BEFORE writing anything. Nothing is written
# unless all pass. No assert is made on a string this patch introduces.
...
for f in (NOW, TRAPS, S3B):
    if not os.path.exists(f):
        print("ABORT — %s not found. Run from ~/abletrace-lab-docs." % f)
        sys.exit(1)

traps = open(TRAPS, encoding="utf-8").read()
s3b   = open(S3B,   encoding="utf-8").read()
...
def check(label, cond, extra=""):
    ...
    if cond:
        ...
        report.append("  FAIL  " + label + ("  [%s]" % extra if extra else ""))
        fail = True

# ---------- TRAPS: tail anchor ----------
TRAPS_TAIL = "and keep every warning line OUTSIDE the fenced command block."
check("TRAPS tail anchor present exactly once",
      traps.count(TRAPS_TAIL) == 1, "found %d" % traps.count(TRAPS_TAIL))
check("TRAPS does not already carry the S91 marker",
      "BROWSER PRINT IS THE JAVA PROCESS" not in traps)

# ---------- 3B.4: build direction ----------
B_OLD = """              A PUSH to main auto-builds PROD.
              A MANUAL DISPATCH takes a `target` input (prod|dev)."""
check("3B.4 build-direction anchor present exactly once",
      s3b.count(B_OLD) == 1, "found %d" % s3b.count(B_OLD))

# ---------- 3B.4: artifact name ----------
A_OLD = "              Artifact name: dist-<target>-<sha>"
check("3B.4 artifact-name anchor present exactly once",
      s3b.count(A_OLD) == 1, "found %d" % s3b.count(A_OLD))

# ---------- 3B.4: rollback points ----------
R_OLD = """CURRENT ROLLBACK POINTS:
  www-html.bak-prod-53db203d4ef4
  www-html.bak-dev-53db203d4ef4"""
check("3B.4 rollback-points anchor present exactly once",
      s3b.count(R_OLD) == 1, "found %d" % s3b.count(R_OLD))

# ---------- 3B.4: checkout-lag line ----------
C_OLD = """⚠ PROD'S FRONTEND GIT CHECKOUT LAGS THE SERVED BUILD. It reads
  9bce0238 while prod SERVES 53db203d."""
check("3B.4 checkout-lag anchor present exactly once",
      s3b.count(C_OLD) == 1, "found %d" % s3b.count(C_OLD))

# ---------- 3B.7: the java/9100 line ----------
J_OLD = """            ⚠ A Java process may occupy port 9100 on Mac startup and
            block Browser Print → sudo lsof -i :9100, kill, reopen."""
check("3B.7 java/9100 anchor present exactly once",
      s3b.count(J_OLD) == 1, "found %d" % s3b.count(J_OLD))

# ---------- 3B.6: certbot email framing ----------
E_OLD = """PROD   ⚠ REGISTERED WITH NO EMAIL. Confirmed via
       `sudo certbot show_account` → "Email contact: none".
       ⚠ SO PROD GETS NO RENEWAL WARNINGS. Worth fixing."""
check("3B.6 prod-certbot anchor present exactly once",
      s3b.count(E_OLD) == 1, "found %d" % s3b.count(E_OLD))

D_OLD = "       ⚠ Registered WITH an email so renewal warnings arrive."
check("3B.6 dev-certbot anchor present exactly once",
      s3b.count(D_OLD) == 1, "found %d" % s3b.count(D_OLD))

print("\n".join(report))
if fail:
    print("\nABORTED — one or more anchors failed. NOTHING WRITTEN.")
    sys.exit(1)
print("\nAll anchors passed. Writing...\n")

# ==================== NOW.md — WHOLE REPLACEMENT ====================
...
open(NOW, "w", encoding="utf-8").write(NOW_NEW)
print("  WROTE  NOW.md  (whole replacement, %d lines)"
      % len(NOW_NEW.splitlines()))

# ==================== TRAPS.md — APPEND ====================

TRAPS_ADD = TRAPS_TAIL + """
```
...
traps = traps.replace(TRAPS_TAIL + "\n```", TRAPS_ADD, 1)
open(TRAPS, "w", encoding="utf-8").write(traps)
print("  WROTE  TRAPS.md  (+5 entries)")

# ==================== Section_3B.md — CORRECTIONS ====================

B_NEW = """              ⚠ A PUSH to main auto-builds DEV, NOT PROD. Corrected
              ...
              A MANUAL DISPATCH takes a `target` input (prod|dev)."""
s3b = s3b.replace(B_OLD, B_NEW, 1)

A_NEW = """              Artifact name: dist-<target>-<sha>
              ⚠ <sha> IS THE FULL 40-CHARACTER SHA, not the short form.
              e.g. dist-prod-275c025039d77a3b88b6e5e1b5671108d33d389a"""
s3b = s3b.replace(A_OLD, A_NEW, 1)

R_NEW = """CURRENT ROLLBACK POINTS (S91):
...
s3b = s3b.replace(R_OLD, R_NEW, 1)

C_NEW = """⚠ PROD'S FRONTEND GIT CHECKOUT LAGS THE SERVED BUILD. It reads
  9bce0238 and has for a long time; prod SERVES whatever was last
  promoted (prod-275c025039d7 as of S91)."""
s3b = s3b.replace(C_OLD, C_NEW, 1)

J_NEW = """            ⚠ THE "java" PROCESS ON 9100 IS BROWSER PRINT ITSELF.
...
s3b = s3b.replace(J_OLD, J_NEW, 1)

E_NEW = """PROD   ⚠ NO EMAIL REGISTERED — AND IT NO LONGER MATTERS.
...
s3b = s3b.replace(E_OLD, E_NEW, 1)

D_NEW = """       ⚠ Registered WITH an email — but this is now WORTHLESS.
       See the PROD note below and P74."""
s3b = s3b.replace(D_OLD, D_NEW, 1)

open(S3B, "w", encoding="utf-8").write(s3b)
print("  WROTE  Section_3B.md  (7 corrections: 3B.4 x4, 3B.6 x2, 3B.7 x1)")

print("\nDONE. Now verify, then commit:")
print("  git add NOW.md TRAPS.md Section_3B.md")
print("  git commit -m 'S91 close: printer findings, P63 closed and promoted to prod, P74 reframed, 3B corrected'")
print("  git push")
