...
**P35  EDITING A PACKING SLIP TO ADD A DISPATCH ORDER THROWS — AND THE FIX IS NOT THE OBVIOUS ONE.** ⚠ NEW S79, found reading code, NOT reported by a user. In `editPackslips` (api/models/PackingSlips.js, block ~325-336) the DO loop references **`elem`, which is out of scope** — it belonged to a `forEach` callback that closed ~15 lines earlier. `ReferenceError` on the first iteration. ⚠ `createPackingSlipDOs` sits AFTER the loop in the same branch, so the DO rows are never written either. ⚠ **OPEN QUESTION FIRST — IS IT REACHABLE?** Does the edit-packing-slip screen ever send a non-empty `req.body.DOs`? One grep of `edit-packslips.component.ts` settles whether this is "does break" or "would break if used". Answer that BEFORE scoping a fix. ⚠⚠ **DO NOT SIMPLY FIX THE `elem` SCOPE.** The throw is currently the only thing stopping three further defects behind it: (a) `Formulations.find()` returns an ARRAY, so `formulation.inventory` is `undefined` and the write is `NaN`; (b) the loop subtracts the same `qty_shipped` for every DO regardless of `i`; (c) it writes `formulations.inventory` — the old Kg line P2 wants retired — for stock that ALREADY left SOH at DO creation (§2 Core #2), i.e. a DOUBLE SUBTRACTION on a live client's inventory. A naive one-line fix turns a loud failure into a silent data corruption. ▶ Correct scope: decide whether this branch should update inventory AT ALL (per §2 it should not), then either delete the loop and keep `createPackingSlipDOs`, or rewrite all four faults together. [§2 to-verify 5 · J85 · 3A.4]
