...
3. **✅ CLOSED S79 — FOUND.** The write is **api/models/MLOManagement.js:154**, one `.set()` carrying everything: `mlc_status: 3`, `startedBy_userId`, `startDate` (from `req.body.mlcStartDate`), and `lotCode`. Line 229 is the close (`mlc_status: 4`). ⚠ Two other `mlc_status: 2` writes exist in MaterialsProductsReleased.js:100 and ReturnMaterialProduct.js:79 — BOTH ARE COMMENTED OUT, dead, not live paths. Do not mistake them for the release write.
4. **✅ CLOSED S79 — VERIFIED.** `SHOW CREATE PROCEDURE WhC_GetFormulaPackagingMaterials` on dev: the SELECT returns `fopackaging.whd_flag` AND `fopackaging.pack_level` explicitly, alongside `wgt_kgs_per_unit`. Logic F's multi-level cascade reads correctly and is not at risk of the silent 1/1/1 failure. No action.
5. **⚠ CORRECTED S79 — IT IS NOT A DEAD BLOCK. IT IS LIVE CODE THAT THROWS.** Read on dev. The block sits in **`editPackslips`** (api/models/PackingSlips.js, fn starts ~245; block ~325-336), inside `if (DOs && DOs.length)`. It references **`elem`, which is out of scope** — `elem` belonged to a `ps.forEach(elem => ...)` callback that closed ~15 lines earlier. An undeclared identifier throws `ReferenceError`, so the loop dies on its first iteration. ⚠ AND `createPackingSlipDOs` SITS AFTER THE LOOP IN THE SAME BRANCH — so it never runs either. Editing a packing slip to ADD a dispatch order cannot succeed. ⚠ **DO NOT "FIX" `elem`.** Repairing the scope would ENABLE three further bugs sitting behind it: `Formulations.find()` returns an ARRAY so `.inventory` is undefined (NaN write); the loop subtracts the same `qty_shipped` for every DO regardless of `i`; and it writes to `formulations.inventory`, the old Kg line, for stock that ALREADY left SOH at DO creation (§2 Core #2) — a double subtraction. The throw is currently the only thing preventing all three. → tracked as P35.
